...
class MessagesHomePage2 extends StatefulWidget {
  const MessagesHomePage2({Key? key}) : super(key: key);

  @override
  State<MessagesHomePage2> createState() => _HomeState();
}

class _HomeState extends State<MessagesHomePage2> {
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  for (var message in messages) {
                    ...
                    var usernames =
                        usersData[messageSender] ?? 'Bilinmeyen Kullanıcı';
                  ...
                  // Veri varsa ListView ile gösteriyoruz
                  /*    return ListView.builder(
                    reverse: true, // En yeni mesaj en üstte olsun
                    itemCount: snapshot.data!.docs.length,
                    itemBuilder: (context, index) {
                      var document = snapshot.data!.docs[index];
                      var email = document['email'];
                      var mesaj = document['mesaj'];

                      // Kullanıcı adını al
                      var username = usersData[email] ?? 'Bilinmeyen Kullanıcı';

                      return ListTile(
                        title: Text('$username: $mesaj'),
                      );
                    },
                  ); */
                }
